...
class CForm implements \ArrayAccess
{
    ...
    public $outputCallback; // Optional callback method to call instead of writing the output
    ...
    public function setOutputCallback($fn) {
     ...
     * @return $this CForm.
     */
    public function addOutput($str, $errors = null)
    {
        ...
        if($this->outputError) {
            $this->outputError[] = ['id' => null, 'label' => null, 'message' => $errors];
        }
     ...
     * @param [type] $keyValueCollection [description]
     */
    public function setValues($keyValueCollection) {

        foreach ($keyValueCollection as $key => $value) {
            if(isset($this[$key])) {
     ...
     * @return mixed the value of the element.
     */
    public function checked($element)
    {
    ...
    public function getHTML($options = [])
    {
        ...
        $novalidate = $options['novalidate']  ? " novalidate='novalidate'" : null;
        ...
        if( $output && is_callable($this->outputCallback)) {
            call_user_func_array($this->outputCallback, [$output, $this->getValidationErrors()]);
            $output = null;
        }
        
        $html = <<< EOD
\n<form{$id}{$class}{$name}{$action}{$method}{$novalidate}>
{$fieldsetStart}
{$legend}
{$elements}
{$output}
...
EOD;
    ...
    public function getValidationErrors()
    {
        ...
        $errors = array_merge($errors, $this->outputError);
